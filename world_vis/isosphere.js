function generate_isosphere_lines(iterations){
	...
	for(let t = 0; t < triangles.length; t++){
		points.push(vertices[triangles[t][0]]);
		points.push(vertices[triangles[t][1]]);
		points.push(vertices[triangles[t][1]]);
		points.push(vertices[triangles[t][2]]);
		points.push(vertices[triangles[t][0]]);
		points.push(vertices[triangles[t][2]]);
	}
...
function generate_isosphere_triangles(iterations){
	let t = (1.0 + Math.sqrt(5.0)) / 2.0;
	let vertices = [
		[-1,t,0],
		[1,t,0],
...
function generate_isosphere_points(iterations, r){
	let t = (1.0 + Math.sqrt(5.0)) / 2.0;
	let vertices = [
		[-1,t,0],
		[1,t,0],
	...
	for(let p = 0; p < vertices.length; p++){
			points.push(vertices[p]);
	}

	for(let p = 0; p < points.length; p++){
		points[p] = mult(points[p], map(Math.random(), 0, 1, r[0], r[1]));
	}
