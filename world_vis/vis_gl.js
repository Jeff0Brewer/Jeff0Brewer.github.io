class Vis{
	constructor(p_fpv, n_fpv, detail){
	...
	draw(u_ModelMatrix){
		...
		//drawing grid
		modelMatrix.translate(0, 0, -.0025);
	...
	update(elapsed, fData){
		...
		for(let i = 0; i < this.change_inds.length; i++){
			let base_i = this.change_inds[i][2];
			let data_i = Math.floor(map(magnitude([this.pos_buffer_grd[base_i - 2], this.pos_buffer_grd[base_i - 1]]), 0, 1, 0, fData.length));
			let height = pow_map(fData[data_i], 0, 255, 0, 2, 2);
