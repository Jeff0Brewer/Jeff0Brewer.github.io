...
function dist_point_segment(point, line){
	let vec = sub(line[0], line[1]);
	let m2 = Math.pow(magnitude(vec), 2);
	if(m2 == 0)
		return dist(point, line[0]);
	let t = ((point[0] - line[0][0]) * (line[1][0] - line[0][0]) + (point[1] - line[0][1]) * (line[1][1] - line[0][1])) / m2;
	t = max(0, min(1, t));
	return dist(point, [line[0][0] + t * (line[1][0] - line[0][0]), line[0][1] + t * (line[1][1] - line[0][1])]);
}

function cube_intersect(point, line, size){
	let vec = sub(line[0], line[1]);
	let m2 = Math.pow(magnitude(vec), 2);
	if(m2 == 0)
		return dist(point, line[0]);
	let t = ((point[0] - line[0][0]) * (line[1][0] - line[0][0]) + (point[1] - line[0][1]) * (line[1][1] - line[0][1])) / m2;
	t = max(0, min(1, t));
	let pt = [line[0][0] + t * (line[1][0] - line[0][0]), line[0][1] + t * (line[1][1] - line[0][1])];
	return Math.abs(point[0] - pt[0]) <= size && Math.abs(point[1] - pt[1]) <= size;
}
...
function smooth(vec, degree, width){
  let out = [];
  for(let i = 0; i < vec.length; i++){
    out.push(vec[i]*degree);
    let bounds = [max(i - width, 0), min(vec.length - 1, i + width)];
    let weight = (1 - degree)/(bounds[1] - bounds[0]);
